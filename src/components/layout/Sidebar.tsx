  ...
  return (
    <aside style={{
      ...
      <div style={{ height: 60, display:'flex', alignItems:'center', justifyContent: collapsed ? 'center' : 'space-between', padding: collapsed ? '0 14px' : '0 16px', borderBottom:'1px solid var(--border)', flexShrink:0 }}>
        {!collapsed && (
          <div style={{ display:'flex', alignItems:'center', gap:9, cursor:'pointer', minWidth:0 }} onClick={()=>navigate('/')}>
            ...
            <div style={{ minWidth:0 }}>
              <div style={{ fontFamily:'Plus Jakarta Sans,sans-serif', fontWeight:800, fontSize:14, lineHeight:1, whiteSpace:'nowrap' }}>
        ...
        {collapsed && (
          <div style={{ width:30, height:30, background:'linear-gradient(135deg,#f97316,#ea580c)', borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', fontSize:15, cursor:'pointer' }} onClick={()=>navigate('/')}>⚡</div>
        )}
        <button onClick={()=>setCollapsed(c=>!c)}
          style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:6, width:22, height:22, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', fontSize:11, color:'var(--text3)', flexShrink:0, padding:0, marginLeft: collapsed ? 0 : 6 }}>
          {collapsed ? '›' : '‹'}
        </button>
      </div>
            ...
            <div key={i}>
              ...
              <div onClick={()=>navigate(item.path)} title={collapsed ? item.label : undefined}
                style={{
                  ...
                  border: `1.5px solid ${isActive ? 'rgba(249,115,22,0.2)' : 'transparent'}`,
                  color: isActive ? '#f97316' : 'var(--text2)',
                  position:'relative',
                }}
                onMouseEnter={e=>{if(!isActive)(e.currentTarget as HTMLElement).style.background='var(--bg2)'}}
                onMouseLeave={e=>{if(!isActive)(e.currentTarget as HTMLElement).style.background='transparent'}}>
                {isActive && !collapsed && (
                  <div style={{ position:'absolute', left:0, top:'20%', bottom:'20%', width:3, background:'#f97316', borderRadius:'0 3px 3px 0' }}/>
                )}
                ...
                {!collapsed && !!item.badge && (
                  <span style={{ background:'#f97316', color:'#fff', fontSize:9, fontWeight:800, padding:'1px 6px', borderRadius:20, minWidth:18, textAlign:'center', flexShrink:0 }}>{item.badge}</span>
                )}
                {collapsed && !!item.badge && (
                  <div style={{ position:'absolute', top:6, right:6, width:7, height:7, background:'#f97316', borderRadius:'50%' }}/>
                )}
              </div>
            </div>
          )
      ...
      <div style={{ borderTop:'1px solid var(--border)', padding:'8px', flexShrink:0 }}>
        <div onClick={toggle} title={dark?'Light mode':'Dark mode'}
          style={{ display:'flex', alignItems:'center', gap: collapsed?0:10, padding: collapsed?'9px':'9px 10px', borderRadius:10, cursor:'pointer', justifyContent: collapsed?'center':'flex-start', marginBottom:2, color:'var(--text2)', transition:'background 0.15s' }}
        ...
        <div onClick={logout} title="Logout"
          style={{ display:'flex', alignItems:'center', gap: collapsed?0:10, padding: collapsed?'9px':'9px 10px', borderRadius:10, cursor:'pointer', justifyContent: collapsed?'center':'flex-start', color:'var(--text2)', transition:'all 0.15s' }}
